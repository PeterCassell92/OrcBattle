...
export const OrcBehaviour = {
    ...
    evaluateCoverPosition(scene, orc, enemies, testX, testY, terrain) {
        ...
        const avgEnemyDistance =
            enemies.reduce((sum, enemy) => sum + Phaser.Math.Distance.Between(testX, testY, enemy.x, enemy.y), 0) /
            enemies.length;
    ...
    calculateBerserkerInvisibilityPosition(scene, orc, enemies) {
        ...
        for (let angle = 0; angle < Math.PI * 2; angle += Math.PI / 8) {
            ...
            positions.push({ x: testX, y: testY, score, angle });
        }
