...
export class King extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y, team) {
    ...
    this.alcoveCenter = { x, y }; // Remember original position
  ...
  checkFlagAreaRestrictions(x, y) {
    ...
    const inBlueFlagArea = x >= blueAlcoveCenter.x - 30
            && x <= blueAlcoveCenter.x + 30
            && y >= blueAlcoveCenter.y - 100
            && y <= blueAlcoveCenter.y - 50;
    const inRedFlagArea = x >= redAlcoveCenter.x - 30
  ...
  launchFireball() {
    ...
    targetX += 20 * randomModifier();
    targetY += 20 * randomModifier();
