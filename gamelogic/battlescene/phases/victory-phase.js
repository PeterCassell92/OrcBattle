...
export function applyVictoryPhaseMethods(SceneClass) {
    SceneClass.prototype.checkWinCondition = function () {
        const blueAlive = this.blueOrcs.filter(orc => orc.active).length;
        const redAlive = this.redOrcs.filter(orc => orc.active).length;

        if ((blueAlive === 0 || redAlive === 0) && !this.gameOver) {
            this.gameOver = true;
            this.gameEndTime = Date.now();

            if (blueAlive === 0) {
                this.winner = 'red';
                this.sacrificeKing('blue');
                this.startVictoryCeremony('red');
            } else {
                this.winner = 'blue';
                this.sacrificeKing('red');
                this.startVictoryCeremony('blue');
            }
        }
    };

    SceneClass.prototype.sacrificeKing = function (team) {
        const king = team === 'blue' ? this.blueKing : this.redKing;
    ...
    SceneClass.prototype.startVictoryCeremony = function (winningTeam) {
        const winningKing = winningTeam === 'blue' ? this.blueKing : this.redKing;
        const winningFlag = winningTeam === 'blue' ? this.blueFlag : this.redFlag;
        const winningNumberText = winningTeam === 'blue' ? this.blueNumberText : this.redNumberText;
        const winningOrcs =
            winningTeam === 'blue' ? this.blueOrcs.filter(orc => orc.active) : this.redOrcs.filter(orc => orc.active);

        // Start burning the losing team's flag
        const losingTeam = winningTeam === 'blue' ? 'red' : 'blue';
        this.burnLosingFlag(losingTeam);

        // Begin royal cleansing of the battlefield - fade bloodstains during ceremony
        this.cleanseBloodstainsDuringVictory();

        this.marchingKing = winningKing;
        this.celebratingOrcs = winningOrcs;
        this.victoryPhase = 'king_marching'; // Track ceremony phase

        // Show victory message
        const winnerText = `${winningTeam === 'blue' ? 'Blue' : 'Red'} Kingdom Conquers!`;
        ...
        this.tweens.add({
            ...
            onStart: () => {
                ...
                const angleToCenter = Phaser.Math.Angle.Between(winningKing.x, winningKing.y, 400, 300);
            ...
            onUpdate: tween => {
                ...
                const angleToTarget = Phaser.Math.Angle.Between(currentX, currentY, remainingTargetX, remainingTargetY);
            ...
            onComplete: () => {
                ...
                console.log(`${winningKing.team} king has reached the center and claims victory!`);

                // King has reached center - start orc celebration
                this.startOrcCelebration(winningTeam);
    ...
    SceneClass.prototype.updateVictoryCeremony = function (time, delta) {
        ...
        if (this.winner && this.marchingKing && this.marchingKing.victoryMarching) {
    ...
    SceneClass.prototype.startOrcCelebration = function (winningTeam) {
        this.victoryPhase = 'orc_celebrating';

        // Get team number for final message
        const teamNumber = winningTeam === 'blue' ? this.blueNumberText.text : this.redNumberText.text;

        // Filter out any null, inactive, or destroyed orcs before starting celebration
        const validCelebratingOrcs = this.celebratingOrcs.filter(orc => {
            return orc && orc.active && orc.body;
        });
        ...
        console.log(`Starting victory celebration with ${validCelebratingOrcs.length} valid orcs`);

        // Update the celebrating orcs list to only include valid ones
        this.celebratingOrcs = validCelebratingOrcs;

        // Limit the number of orcs that speak based on survivors
        const maxSpeakingOrcs = Math.min(this.celebratingOrcs.length, 3); // Max 3 orcs speak
        const speakingOrcs = this.celebratingOrcs.slice(0, maxSpeakingOrcs); // Take first N orcs

        // Arrange orcs in two vertical lines flanking the king
        this.celebratingOrcs.forEach((orc, index) => {
            ...
            const startY = kingY - (this.celebratingOrcs.length / 2) * (orcSpacing / 2); // Center the line vertically
            ...
            this.tweens.add({
                ...
                onComplete: () => {
                    ...
                    const angleToKing = Phaser.Math.Angle.Between(orc.x, orc.y, kingX, kingY);
                    ...
                    if (speakingOrcs.includes(orc)) {
                        ...
                        this.startOrcSpeech(orc, teamNumber, speakingIndex);
                    }
                },
            });
        });
    ...
    SceneClass.prototype.burnLosingFlag = function (losingTeam) {
        const losingFlag = losingTeam === 'blue' ? this.blueFlag : this.redFlag;
        const losingNumberText = losingTeam === 'blue' ? this.blueNumberText : this.redNumberText;

        // Start with the flag catching fire
        console.log(`${losingTeam} flag begins to burn!`);

        // Create fire particles around the flag
        this.createFireAnimation(losingFlag, losingNumberText);
    ...
    SceneClass.prototype.createFireAnimation = function (flag, numberText) {
        ...
        const createFireParticle = () => {
            ...
            if (!this || !this.gameOver || !this.add || !flag || this.disableFireParticles) {
                return; // Exit if scene or flag is invalid, or fire particles are disabled
            }

            try {
                // Validate flag position
                if (typeof flag.x !== 'number' || typeof flag.y !== 'number' || isNaN(flag.x) || isNaN(flag.y)) {
                ...
                if (particleX < -100 || particleX > 900 || particleY < -100 || particleY > 700) {
    ...
    SceneClass.prototype.cleanseBloodstainsDuringVictory = function () {
        ...
        if (!this.bloodStains || this.bloodStains.length === 0) {
            console.log('No blood stains to cleanse - battlefield already pristine for royal celebration');
            return;
        }
        ...
        this.createRoyalCleansingEffect();

        // Rapidly advance each bloodstain through stages during royal cleansing
        this.bloodStains.forEach(async (bloodStain, index) => {
        ...
        setTimeout(() => {
            ...
            this.bloodStains = this.bloodStains.filter(stain => stain && stain.active);

            if (this.bloodStains.length === 0) {
                console.log('Royal cleansing complete: battlefield is now pristine for the victory celebration!');
            } else {
                console.log(`Royal cleansing in progress: ${this.bloodStains.length} bloodstains still fading`);
            }
    ...
    SceneClass.prototype.createRoyalCleansingEffect = function () {
        ...
        if (!this || !this.add || !this.tweens || this.sys?.isDestroyed?.()) {
        ...
        this.cleansingTimeouts = this.cleansingTimeouts || [];
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            const timeoutId = setTimeout(() => {
                ...
                if (this && this.add && !this.sys?.isDestroyed?.()) {
                    this.createCleansingSparkle(x, y);
                }
            }, i * 100); // 100ms between each sparkle

            // Store timeout ID for potential cleanup
            this.cleansingTimeouts.push(timeoutId);
        }
    };
    ...
    SceneClass.prototype.createCleansingSparkle = function (x, y) {
        ...
        if (!this || !this.add || !this.tweens || this.sys?.isDestroyed?.()) {
            console.warn('Cannot create cleansing sparkle - scene is invalid or destroyed');
            return;
        }

        try {
            // Create a golden sparkle that represents royal cleansing magic
            const sparkle = this.add.star(x, y, 4, 3, 6, 0xffd700); // Golden 4-pointed star
            ...
            this.tweens.add({
                ...
                onComplete: () => {
                    ...
                    if (sparkle && sparkle.active && this && this.tweens && !this.sys?.isDestroyed?.()) {
            ...
            if (this && this.tweens && !this.sys?.isDestroyed?.()) {
                this.tweens.add({
                    targets: sparkle,
                    scaleX: 1.2,
                    scaleY: 1.2,
